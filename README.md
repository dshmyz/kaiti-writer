...
| **生成 Word** | 自动套学校官方模板，格式、字号、行距、标题层级、目录页码全都对，不用你调 |
| **生成 PPT** | 帮你做汇报用的答辩 PPT，12-15 页，自动从 Word 内容里提炼要点 |
| **准备答辩** | 帮你写演讲稿（逐字稿）、预测评委可能问的 10-15 个问题并教你怎么答 |
| **模拟拷问** | AI 扮演北航评委，连问 5 轮、追问到底，答模糊了当场揪出来，帮你提前暴露薄弱点 |
...
写正文（一节一节写 → 每节去AI味）
    ↓
生成 Word（套学校模板 → 标题层级/目录页码/格式归一化）
    ↓
自查（检查格式/内容/占位 → 给你清单）
    ↓
收尾（PPT + 演讲稿 + 答辩问答 → 全部搞定）
    ↓
拷问（AI扮演评委 → 5轮追问 → 暴露薄弱点）
```
...
默认是**北航 MPA（公共管理）**的格式和模板，严格按照《开题报告格式规范》排版（三号黑体标题、小四宋体正文、1.5倍行距、自动目录页码等）。如果你是别的学校，可以自己换模板——技能会自动识别你的模板格式。
...
│   ├── 选题引导.md             # 怎么帮你选题目
│   ├── 文献检索.md             # 怎么帮你搜文献
│   ├── 格式规范.md             # 北航开题报告格式要求（字号/行距/标题层级）
│   ├── 篇幅档位.md             # 写多长（精简/标准/充实）
│   ├── 去AI化写作守则.md       # 怎么去掉AI味
│   ├── 答辩拷问.md             # 模拟答辩拷问流程（AI扮演评委，5轮追问）
│   └── ...
├── scripts/                    # 自动生成文件的脚本
│   ├── build_from_template.py  # 生成 Word（标题层级/目录页码/格式归一化）
│   ├── build_ppt_from_template.py  # 生成 PPT
│   ├── derive_ppt_content.py   # 从 Word 内容自动提炼 PPT 要点
│   ├── fix_refs.py             # 修复参考文献格式（年份补全/悬挂缩进）
